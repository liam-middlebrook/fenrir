...
int instanceCountW = 25;
int instanceCountH = 25;
int instanceCount = instanceCountW  * instanceCountH;
Mesh::Mesh(GLfloat* vertices, uint vertexCount, GLuint* indices, uint indexCount, GLuint shaderProgram)
{
	...
	this->worldMatrices = new glm::mat4[instanceCount];

	for (int i = 0; i < instanceCountH; i++)
	{
		for (int j = 0; j < instanceCountW; j++)
		{
			worldMatrices[i*instanceCountW+j] = glm::translate(glm::mat4(1), glm::vec3(4*i, 4*j, 0));
		}
	}
...
void Mesh::DrawMesh(glm::mat4 viewMatrix, glm::mat4 projectionMatrix, bool outline)
{
	...
	glUniformMatrix4fv(worldUni, instanceCount, GL_FALSE, (const GLfloat*)(worldMatrices));
